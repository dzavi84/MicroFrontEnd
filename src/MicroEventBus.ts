...
export class MicroEventBus {
  ...
  on<T extends keyof IDomainEventKey>(
    ...
    console.log(`BCStT: on(${key})`);
    return this.eventBus.asObservable().pipe(
      filter((event: IBroadcastEvent<T>) => event.key === key),
      map((event: IBroadcastEvent<T>) => event.data),
    );
  }

  broadcast<T extends keyof IDomainEventKey>(key: T, data: IDomainEventKey[T]) {
